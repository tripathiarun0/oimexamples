...
public class JMSClientTester extends BaseJMSClient {
   ...
   public static void main(String args[]) {
       ...
       JMSClientTester spmlc = new JMSClientTester();
   ...
   public void sendMessage(HashMap messageData) {
       ...
       try {
           qcf = (QueueConnectionFactory)ctx.lookup(getProperty("JMSQFACTORY_NAME"));
       }
       catch (NamingException ne) {
           logger.error(ne.getMessage(), ne);
           System.exit(0);
       ...
       // lookup Queue
       try {
           q = (Queue) ctx.lookup(getProperty("AIJMS_QUEUE_NAME"));
       }
       catch (NamingException ne) {
           logger.error(ne.getMessage(), ne);
           System.exit(0);
